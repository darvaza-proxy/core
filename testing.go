...
type MockT struct {
	// Errors contains all error messages that would have been reported
	Errors []string
	// Logs contains all log messages that would have been reported
	Logs []string
	// HelperCalled tracks whether Helper() was called
	HelperCalled bool
}
...
// LastError returns the last error message, or empty string if no errors.
...
// Reset clears all recorded errors, logs, and helper state.
func (m *MockT) Reset() {
	m.Errors = nil
	m.Logs = nil
	m.HelperCalled = false
}
